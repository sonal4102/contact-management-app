
const Popup = ({ close, el }) => {



    return (
        <div className="fixed top-0    text-black left-0 w-full h-full  bg-opacity-20 bg-black flex items-center justify-center">
            <div className="bg-white drop-shadow-md rounded-md p-4 w-mid sm:w-1/4 shadow-lg">
                <div className="flex justify-between items-center mb-2">
                    <h2 className="text-lg font-medium">Contact Details</h2>
                    <button className="text-gray-100 px-2 rounded-md bg-orange-300 hover:text-gray-800" onClick={() => close()}>
                        X
                    </button>
                </div>
                <div key={el.id} className="bg-gray-300  m-4 p-4 text-black">
                    <div className="w-3/4 m-auto  ">
                        <img className="w-full rounded-full" src="data:image/png;base64," alt="" />
                        <div className="p-4">
                        </div>
                    </div>
                    <div className="text-left ">
                    <p className="font-semibold">First Name: <span className="font-normal">{el.first_name}</span></p>

                        <p className="font-semibold">Last Name  : <span className="font-normal">{el.last_name}</span></p>
                        <p className="font-semibold">Mobile No  :<span className="font-normal">{el.mob}</span> </p>
                        <p className="font-semibold">Status       :    <span className="font-normal"> {el.status == "active" ? "Active" : "Inactive"}</span> </p>
                    </div>


                </div>
            </div>
        </div>
    );
}

export default Popup
